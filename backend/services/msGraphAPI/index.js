    ...
    try {
        ...
        if (!msResponse.error)
            return res.status(200).json({
                success: 1, code: 'msGetFile.ok', data: {
    ...
    try {
        ...
        if (!msResponse.error)
            return res.status(201).json({
                success: 1, code: 'msUploadSmall.ok', data: {
                    size: msResponse.size,
                    id: msResponse.id,
                    filename: msResponse.name,
                }
...
async (req, res, next) => {
    ...
    try {
        ...
        if (!msResponse.error) {
            ...
            return res.status(200).json({
                success: 1, code: 'msUploadBig.ready', data: {
                    size: filesize,
                    id: fileId,
                    filename: filename,
    ...
    try {
        ...
        if (!msResponse.error)
            return res.status(202).json({
                success: 1,
                code: 'msSendMail.success',
                message: 'mail.json successfully sent.'
            });
        else
            return res.status(msResponse.status).json({
                error: 1,
