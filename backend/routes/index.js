...
async (req, res, next)=>{
    try {
        ...
        if(req.query.cheaters==='')
            data.cheaters = (await db('cheaters').count({cheaters: 'id'}).where('createTime', '>=', new Date(from)).andWhere({valid: 1}) )[0].cheaters;
        if(req.query.confirmed==='')
            data.confirmed = (await db('cheaters').count({confirmed: 'id'}).where('createTime', '>=', new Date(from)).andWhere({status: 1}).andWhere({valid: 1}) )[0].confirmed;
        ...
        if(req.query.details==='') {
            ...
            for(let i of config.supportGames)
                data.details.bygame[i] = (await db('cheaters').count({game: 'id'}).where('games', 'like', `%${i}%`).andWhere({valid: 1}).andWhere('createTime', '>=', new Date(from)) )[0].game;
            for(let i of ['1','2','3','4','5','6'])
                data.details.bystatus[i] = (await db('cheaters').count({status: 'id'}).where({status: i}).andWhere({valid: 1}).andWhere('createTime', '>=', new Date(from)) )[0].status;
        }
